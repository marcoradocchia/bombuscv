...
def get_dims(
    cap: cv.VideoCapture, res: Tuple[int, int] = "720p"
) -> Tuple[int, int]:
...
def set_cap_props(
    cap: cv.VideoCapture, dims: Tuple[int, int], fps: int
) -> None:
...
def main() -> None:
    ...
    dims = get_dims(cap=cap, res=res)
    video_type = get_video_type(filename)
    set_cap_props(cap=cap, dims=dims, fps=fps)
